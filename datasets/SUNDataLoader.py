...
class SUNDataLoader():
    ...
    def read_matdataset(self):
        ...
        self.attribute = self.att
    
    def construct_torch_dataset(self, config):
        bs = config.batch_size
        train_torch_set = torch.utils.data.TensorDataset(self.train_feature)
        test_seen_set = torch.utils.data.TensorDataset(self.test_seen_feature)
        test_unseen_set = torch.utils.data.TensorDataset(self.test_unseen_feature)
        train_torch_loader = torch.utils.data.DataLoader(train_torch_set, batch_size=bs)
        test_seen_torch_loader = torch.utils.data.DataLoader(test_seen_set, batch_size=bs)
        test_unseen_torch_loader = torch.utils.data.DataLoader(test_unseen_set, batch_size=bs)
        self.train_torch_loader = train_torch_loader
        self.test_seen_torch_loader = test_seen_torch_loader
        self.test_unseen_torch_loader = test_unseen_torch_loader
